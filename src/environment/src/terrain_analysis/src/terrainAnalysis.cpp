...
//把点分配到山歌中
int terrainVoxelUpdateNum[terrainVoxelNum] = {0};//山歌中点的数量
...
void joystickHandler(const sensor_msgs::Joy::ConstPtr &joy) {
  if (joy->buttons[7] > 0.5) {
    noDataInited = 0;
    clearingCloud = true;
  }
}

// cloud clearing callback function 这个话题没有发布者
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      for (int i = 0; i < laserCloudCropSize; i++) {
        point = laserCloudCrop->points[i];//就是register_pointcloud lio-sam给的
