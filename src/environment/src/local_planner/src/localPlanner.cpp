...
void joystickHandler(const sensor_msgs::Joy::ConstPtr& joy)
{
  ...
  //   checkObstacle = false;
  // }
  joySpeedRaw = sqrt(joy->axes[2] * joy->axes[2] + joy->axes[3] * joy->axes[3]);
  joySpeed = joySpeedRaw;
  if (joySpeed > 1.0) joySpeed = 1.0;
  if (joy->axes[3] == 0) joySpeed = 0;

  if (joySpeed > 0) {
    joyDir = atan2(joy->axes[2], joy->axes[3]) * 180 / PI;
    if (joy->axes[3] < 0) joyDir *= -1;
  }

  if (joy->axes[3] < 0 && !twoWayDrive) joySpeed = 0;
