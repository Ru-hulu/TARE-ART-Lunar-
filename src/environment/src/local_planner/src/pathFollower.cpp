...
void joystickHandler(const sensor_msgs::Joy::ConstPtr& joy)
{
  ...
  joySpeedRaw = sqrt(joy->axes[3] * joy->axes[3] + joy->axes[2] * joy->axes[2]);
  joySpeed = joySpeedRaw;
  if (joySpeed > 1.0) joySpeed = 1.0;
...
int main(int argc, char** argv)
{
  ...
  while (status) {
    ros::spinOnce();
    // cct = (cct+1)%100;
    // if(cct == 0 && autonomyMode)
    // std::cout<<"now is auto !!"<<joySpeed<<std::endl;      
    ...
    if (pathInit) {
      ...
      if (odomTime < stopInitTime + stopTime && stopInitTime > 0) {
        vehicleSpeed = 0;
        vehicleYawRate = 0;
        std::cout<<"stop here 0"<<std::endl;
      }

      if (safetyStop >= 1)
      {
        vehicleSpeed = 0;
        std::cout<<"stop here 1"<<std::endl;
      }
      if (safetyStop >= 2)
      {
       vehicleYawRate = 0;
        std::cout<<"stop here 2"<<std::endl;
      }
